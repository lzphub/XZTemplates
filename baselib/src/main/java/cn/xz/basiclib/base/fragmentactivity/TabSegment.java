...
import cn.xz.basiclib.util.ColorHelper;
import cn.xz.basiclib.util.DisplayHelper;
import cn.xz.basiclib.util.DrawableHelper;
import cn.xz.basiclib.util.LangHelper;
import cn.xz.basiclib.util.ResHelper;
import cn.xz.basiclib.util.ViewHelper;
    ...
    protected OnClickListener mTabOnClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (mOnTabClickListener != null) {
                isContinueClick = mOnTabClickListener.onTabClick(index);
            }
...
//    private AdapterChangeListener mAdapterChangeListener;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mDefaultSelectedColor = ResHelper.getAttrColor(context, R.attr.config_color_blue);
        mDefaultNormalColor = ContextCompat.getColor(context, R.color.config_color_gray_5);
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.TabSegment, defStyleAttr, 0);
        mHasIndicator = array.getBoolean(R.styleable.TabSegment_tab_has_indicator, true);
        mIndicatorHeight = array.getDimensionPixelSize(R.styleable.TabSegment_tab_indicator_height,
                getResources().getDimensionPixelSize(R.dimen.tab_segment_indicator_height));
        ...
        mIndicatorTop = array.getBoolean(R.styleable.TabSegment_tab_indicator_top, false);
        mDefaultTabIconPosition = array.getInt(R.styleable.TabSegment_tab_icon_position, ICON_POSITION_LEFT);
        mMode = array.getInt(R.styleable.TabSegment_tab_mode, MODE_FIXED);
        mItemSpaceInScrollMode = array.getDimensionPixelSize(R.styleable.TabSegment_tab_space, DisplayHelper.dp2px(context, 10));
        typefaceProviderName = array.getString(R.styleable.TabSegment_tab_typeface_provider);
        array.recycle();
    ...
    private void createTypefaceProvider(Context context, String className) {
        if (LangHelper.isNullOrEmpty(className)) {
            return;
        }
    ...
    private void createIndicatorView() {
        if (mIndicatorView == null) {
            ...
            if (mIndicatorDrawable != null) {
                ViewHelper.setBackgroundKeepingPadding(mIndicatorView, mIndicatorDrawable);
            } else {
                if (mIndicatorSelectedColor!=0){
                    mIndicatorView.setBackgroundColor(mIndicatorSelectedColor);
                }else {
    ...
    public void setIndicatorColor(@ColorInt int indicatorColor) {
        mIndicatorSelectedColor = indicatorColor;
        mContentLayout.invalidate();
    }
...
//            viewPager.addOnAdapterChangeListener(mAdapterChangeListener);
        ...
        if (mSelectedIndex == Integer.MIN_VALUE) {
            ...
            if (mIndicatorView != null && listViews.size() > 1) {
                if (mIndicatorDrawable != null) {
                    ...
                    mIndicatorView.setBackgroundColor(getIndicatorSelectedColor(model));
                }
            }
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if (mIndicatorView != null && listViews.size() > 1) {
                    ...
                    if (mIndicatorDrawable == null) {
                        mIndicatorView.setBackgroundColor(ColorHelper.computeColor(getIndicatorSelectedColor(prevModel), getIndicatorSelectedColor(nowModel), animValue));
                    }
                    mIndicatorView.layout(targetLeft, mIndicatorView.getTop(), targetLeft + targetWidth, mIndicatorView.getBottom());
                }
                int preColor = ColorHelper.computeColor(getTabSelectedColor(prevModel), getTabNormalColor(prevModel), animValue);
                int nowColor = ColorHelper.computeColor(getTabNormalColor(nowModel), getTabSelectedColor(nowModel), animValue);
    ...
    public void updateIndicatorPosition(int index, float offsetPercent) {
        ...
        int preColor = ColorHelper.computeColor(getTabSelectedColor(preModel), getTabNormalColor(preModel), offsetPercent);
        int targetColor = ColorHelper.computeColor(getTabNormalColor(targetModel), getTabSelectedColor(targetModel), offsetPercent);
        ...
        if (mIndicatorView != null && listViews.size() > 1) {
            ...
            if (mIndicatorDrawable == null) {
                int indicatorColor = ColorHelper.computeColor(getIndicatorSelectedColor(preModel), getIndicatorSelectedColor(targetModel), offsetPercent);
    ...
    private int getIndicatorSelectedColor(Tab item) {
        int color = item.getSelectedColor();
        if (color == Tab.USE_TAB_SEGMENT) {
            color = mIndicatorSelectedColor!=0?mIndicatorSelectedColor:mDefaultSelectedColor;
        }
        return color;
    }
    ...
    public static class Tab {
        ...
        private TextView ensureSignCountView(Context context) {
            if (mSignCountTextView == null) {
                mSignCountTextView = new TextView(context, null, R.attr.tab_sign_count_view);
                RelativeLayout.LayoutParams signCountLp = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ResHelper.getAttrDimen(context, R.attr.tab_sign_count_view_minSize));
        ...
        public void showSignCountView(Context context, int count) {
            ...
            if (count != 0) {
                ...
                mSignCountTextView.setMinWidth(ResHelper.getAttrDimen(mSignCountTextView.getContext(), R.attr.tab_sign_count_view_minSize));
        ...
        private String getNumberDigitsFormattingValue(int number) {
            if (LangHelper.getNumberDigits(number) > mSignCountDigits) {
                String result = "";
        ...
        protected void bind(Tab item, TabItemView view, int position) {
            ...
            if (item.getNormalIcon() == null) {
                ...
                if (drawable != null) {
                    ...
                    tv.setCompoundDrawablePadding(DisplayHelper.dp2px(getContext(), 4));
                } else {
                    tv.setCompoundDrawables(null, null, null, null);
                }
            }
            ...
            if (position == mSelectedIndex) {
                if (mIndicatorView != null && getViews().size() > 1) {
                    if (mIndicatorDrawable != null) {
                        ...
                        mIndicatorView.setBackgroundColor(getIndicatorSelectedColor(item));
                    }
                }
